...
cmd_setup_env() {
    # Idempotent .env generator for CI/CD
    # - Creates .env if it doesn't exist
    # - Adds any missing keys to an existing .env (never removes existing values)
    # - Generates secure random values for passwords/secrets

    # Helper to generate random passwords (16 chars, alphanumeric)
    gen_pass() {
        openssl rand -base64 24 2>/dev/null | tr -d '/+=' | head -c 16 || \
        cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 16 | head -n 1
    }

    # Helper to generate hex secrets (64 chars for Authentik secret key)
    gen_hex_secret() {
        local length="${1:-32}"
        openssl rand -hex "$length" 2>/dev/null || \
        cat /dev/urandom | tr -dc 'a-f0-9' | fold -w "$((length * 2))" | head -n 1
    }
...
HEADER
    ...
    # ensure_key KEY DEFAULT_VALUE
    # Adds KEY=DEFAULT_VALUE to .env only if KEY is not already present
    ensure_key() {
        local key="$1"
        local value="$2"
        if ! grep -q "^${key}=" "$ENV_FILE" 2>/dev/null; then
            echo "${key}=${value}" >> "$ENV_FILE"
            print_info "  Generated missing key: ${key}"
        fi
    }

    print_info "Checking .env for missing keys..."

    local missing_before
    missing_before=0

    # --- General ---
    ensure_key "TZ"                        "${TZ:-America/Toronto}"
    ensure_key "PUID"                      "${PUID:-1001}"
    ensure_key "PGID"                      "${PGID:-1001}"

    # --- PhotoPrism ---
    ensure_key "PHOTOPRISM_ADMIN_PASSWORD" "$(gen_pass)"
    ensure_key "PHOTOPRISM_DB_NAME"        "photoprism"
    ensure_key "PHOTOPRISM_DB_USER"        "photoprism"
    ensure_key "PHOTOPRISM_DB_PASSWORD"    "$(gen_pass)"

    # --- Nextcloud ---
    ensure_key "NEXTCLOUD_DB_NAME"         "nextcloud"
    ensure_key "NEXTCLOUD_DB_USER"         "nextcloud"
    ensure_key "NEXTCLOUD_DB_PASSWORD"     "$(gen_pass)"
    ensure_key "NEXTCLOUD_ADMIN_USER"      "admin"
    ensure_key "NEXTCLOUD_ADMIN_PASSWORD"  "$(gen_pass)"

    # --- Authentik ---
    ensure_key "AUTHENTIK_SECRET_KEY"       "$(gen_hex_secret 32)"
    ensure_key "AUTHENTIK_POSTGRES_USER"    "authentik"
    ensure_key "AUTHENTIK_POSTGRES_PASSWORD" "$(gen_pass)"

    # --- Network / Misc ---
    ensure_key "SULLIVAN_TAILSCALE_IP"     "${SULLIVAN_TAILSCALE_IP:-100.87.125.19}"
    ensure_key "MEDIA_PATH_AUDIOBOOKS"     "${MEDIA_PATH_AUDIOBOOKS:-/mnt/1tb/audiobookshelf/audiobooks}"

    chmod 600 "$ENV_FILE"

    local key_count
    key_count=$(grep -c '^[A-Z_]' "$ENV_FILE" 2>/dev/null || echo "0")
    print_success ".env ready ($key_count keys)"
}
